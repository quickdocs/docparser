...
(defmacro with-ignored-errors (() &body body)
  ...
  `(handler-case
     ...
     (asdf:load-system-definition-error (c)
       (format *error-output* "Error when loading the system definition. Ignoring.~%  ~A~%" c))
     (error (c)
       (let ((condition (class-name (class-of c))))
         (if (and (string= (package-name (symbol-package condition)) :cffi)
             ...
             (format *error-output* "Failed to load foreign library. Ignoring.~%  ~A~%" c)
             (format *error-output* "Unknown error: ~A~%" c))))))
